...
const SignUpPage: React.FC = () => {
  ...
  const handlePasswordBlur = () => {
    ...
    if (password && !validatePassword(password)) {
      setPasswordError("Password minimal 8 karakter, mengandung minimal 1 angka dan 2 huruf.");
    } else {
      setPasswordError("");
    }
  };
      ...
      <div className="w-full max-w-sm p-8 bg-white rounded-2xl shadow">
        ...
        <form onSubmit={handleSignUp} className="flex flex-col gap-4">
          <input
            ...
            required
          />
          {emailError && emailTouched && <div className="text-red-600 text-sm mt-0 -mb-2">{emailError}</div>}
          ...
          {passwordError && passwordTouched && <div className="text-red-600 text-sm mt-0 -mb-2">{passwordError}</div>}
          <button
            type="submit"
